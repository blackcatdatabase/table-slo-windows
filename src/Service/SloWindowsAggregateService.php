...
namespace BlackCat\Database\Packages\SloWindows\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto;
use BlackCat\Database\Packages\SloWindows\Mapper\SloWindowDtoMapper;
 ...
 */
final class SloWindowsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SloWindowRepository $sloWindowRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'slo_windows' => $this->sloWindowRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SloWindows\ModuleException("Record {$idStr} not found for locking.");
            }
