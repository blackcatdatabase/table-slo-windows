...
namespace BlackCat\Database\Packages\SloWindows\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SloWindows\Definitions;
use BlackCat\Database\Packages\SloWindows\Criteria;
use BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto as Dto;
use BlackCat\Database\Packages\SloWindows\Mapper\SloWindowDtoMapper as RowMapper;
...
final class SloWindowRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SloWindows\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'objective', 'target_pct', 'window_interval' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SloWindows\Definitions::table());
            return $bulk->upsertMany($rows, [ 'name' ], [ 'objective', 'target_pct', 'window_interval' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto|null */
    public function getByName(string $name, bool $asDto = false): array|\BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto|null {
        $row = $this->getByUnique([ 'name' => $name ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SloWindows\Mapper\SloWindowDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto|null */
    public function getByConstraint(mixed $constraint, bool $asDto = false): array|\BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto|null {
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SloWindows\Dto\SloWindowDto|null {
