...
use BlackCat\Database\Packages\SloWindows\Repository\SloWindowRepositoryInterface;
...
class SloWindowRepository implements SloWindowRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\SloWindows\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'objective', 'target_pct', 'window_interval' ]);
          return count($rows);
        }
